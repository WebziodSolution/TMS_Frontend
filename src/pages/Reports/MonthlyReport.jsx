...
import { getMonthlyReport, exportMonthlyReportExcel } from '../../services/reportService';
...
const MonthlyReport = ({ setAlert }) => {
    ...
    const [selectedTab, setSelectedTab] = useState(0); // 0 = Ticket View, 1 = Developer View
    ...
    const fetchReport = async (startStr, endStr, groupBy = selectedTab === 0 ? "ticket" : "developer") => {
        if (!hasAccess) return;
        setLoading(true);
        try {
            const res = await getMonthlyReport(startStr, endStr, groupBy);
            if (res.status === 200) {
        ...
        if (startDate && endDate && dayjs(startDate).isValid() && dayjs(endDate).isValid()) {
            if (startDate.isAfter(endDate)) {
                ...
                fetchReport(
                    ...
                    selectedTab === 0 ? "ticket" : "developer"
                );
            }
        }
    ...
    const handleExportExcel = async () => {
        if (!startDate || !endDate || !dayjs(startDate).isValid() || !dayjs(endDate).isValid()) {
            setAlert({ open: true, message: "Please select a valid date range first.", type: "error" });
            return;
        }

        setExportLoading(true);
        try {
            const blob = await exportMonthlyReportExcel(
                startDate.format('YYYY-MM-DD'),
                endDate.format('YYYY-MM-DD'),
                selectedTab === 0 ? "ticket" : "developer"
            );
            ...
            link.href = url;
            const filename = `monthly_report_${selectedTab === 0 ? "ticket" : "developer"}_${startDate.format('YYYY-MM-DD')}_to_${endDate.format('YYYY-MM-DD')}.xlsx`;
            ...
            // setAlert({ open: true, message: "Excel report exported successfully!", type: "success" });
        } catch (err) {
            console.error(err);
            setAlert({ open: true, message: "An error occurred while exporting the report.", type: "error" });
        } finally {
    ...
    if (selectedTab === 0) {
        ...
        reportData.forEach(dev => {
            const { hours, minutes } = parseWorkingHours(dev.total_working_hours);
            totalMinutesLogged += hours * 60 + minutes;
        });
    }
    ...
    return (
        <div className="w-full max-w-7xl mx-auto space-y-8 animate-fade-in-up pb-10">
            {/* Header and Controls */}
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-6 border border-[#DFE1E6] rounded-2xl shadow-sm mb-3">
                <div>
                ...
                <div className="flex flex-wrap items-center gap-3 self-start md:self-center">
                    ...
                    <button
                        ...
                        className="bg-green-600 hover:bg-green-700 text-white font-bold text-sm px-4 py-2.5 rounded-xl transition active:scale-95 cursor-pointer h-[40px] flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        {exportLoading ? (
            ...
            <div className="border-b border-[#DFE1E6]">
                <Tabs
                    ...
                    tabsData={[
                        { label: 'Ticket View', icon: <FontAwesomeIcon icon={faTicketAlt} /> },
                        { label: 'Developer View', icon: <FontAwesomeIcon icon={faUsers} /> }
                    ]}
                    fontSize="16px"
                />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                {/* Tickets/Developers Card */}
                <div className="relative overflow-hidden p-6 bg-white border border-[#DFE1E6] rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-0.5">
                    <div className="flex justify-between items-start">
                        <div>
                            <h3 className="font-medium text-[#5E6C84] text-xs uppercase tracking-wider">
                                {selectedTab === 0 ? "Total Tickets" : "Total Developers"}
                            </h3>
                            <p className="text-3xl font-bold mt-2 text-[#172B4D]">{reportData.length}</p>
                        </div>
                        <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${selectedTab === 0 ? "bg-[#FFF0B3] text-[#FF991F]" : "bg-[#EAE6FF] text-[#403294]"
                            }`}>
                            <FontAwesomeIcon icon={selectedTab === 0 ? faTicketAlt : faUsers} size="lg" />
                        </div>
                    </div>
                    <div className={`absolute bottom-0 left-0 w-full h-1 ${selectedTab === 0 ? "bg-[#FF991F]" : "bg-[#403294]"
                        }`} />
                </div>
            </div>
                    ...
                    selectedTab === 0 ? (
                        ...
                        <div className="space-y-4">
                            ...
                            {reportData.map((ticket) => {
                                ...
                                const { hours: actHours, minutes: actMinutes } = parseWorkingHours(ticket.total_actual_hours);
                                const ticketActualTimeDisplay = formatLoggedTime(actHours, actMinutes);
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center justify-between md:justify-end gap-5 shrink-0">
                                                <div className="flex gap-4 text-right">
                                                    <div>
                                                        <div className="text-xs text-[#5E6C84] font-medium">Total Worked Time</div>
                                                        <div className="text-base font-bold text-[#172B4D]">{ticketTimeDisplay}</div>
                                                    </div>
                                                    <div>
                                                        <div className="text-xs text-[#5E6C84] font-medium">Total Actual Time</div>
                                                        <div className="text-base font-bold text-slate-600">{ticketActualTimeDisplay}</div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="overflow-hidden border border-[#DFE1E6] rounded-xl bg-white shadow-xs">
                                                    <table className="min-w-full divide-y divide-[#DFE1E6]">
                                                        <thead className="bg-[#FAFBFC]">
                                                            <tr>
                                                                <th scope="col" className="px-5 py-3 text-left text-xs font-bold text-[#8993A4] uppercase tracking-wider border-l border-[#DFE1E6]">Developer Name</th>
                                                                <th scope="col" className="px-5 py-3 text-right text-xs font-bold text-[#8993A4] uppercase tracking-wider border-l border-[#DFE1E6]">Worked Time</th>
                                                                <th scope="col" className="px-5 py-3 text-right text-xs font-bold text-[#8993A4] uppercase tracking-wider border-l border-[#DFE1E6]">Actual Time</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody className="bg-white divide-y divide-[#DFE1E6]">
                                                            {(ticket.users || []).map((user) => {
                                                                ...
                                                                const { hours: uaHrs, minutes: uaMins } = parseWorkingHours(user.actual_working_hours);
                                                                const userActualTimeDisplay = formatLoggedTime(uaHrs, uaMins);

                                                                return (
                                                                    <tr key={user.developer_id} className="hover:bg-[#FAFBFC] transition-colors">
                                                                        <td className="px-5 py-3.5 whitespace-nowrap text-sm font-semibold text-[#172B4D] border-l border-[#DFE1E6]">
                                                                            <div className="flex items-center gap-2">
                                                                        ...
                                                                        <td className="px-5 py-3.5 whitespace-nowrap text-sm text-[#172B4D] font-semibold text-right border-l border-[#DFE1E6]">
                                                                            {userTimeDisplay}
                                                                        </td>
                                                                        <td className="px-5 py-3.5 whitespace-nowrap text-sm text-slate-600 font-semibold text-right border-l border-[#DFE1E6]">
                                                                            {userActualTimeDisplay}
                                                                        </td>
                                                                    </tr>
                                                                );
                                                            })}
                        ...
                        <div className="space-y-4">
                            <h3 className="text-lg font-bold text-[#172B4D] px-1">Developer Work Breakdown</h3>

                            {reportData.map((developer) => {
                                const isExpanded = !!expandedTickets[developer.developer_id];

                                // Parse and format total logged time for this developer
                                const { hours, minutes } = parseWorkingHours(developer.total_working_hours);
                                const devTimeDisplay = formatLoggedTime(hours, minutes);

                                const { hours: actHours, minutes: actMinutes } = parseWorkingHours(developer.total_actual_hours);
                                const devActualTimeDisplay = formatLoggedTime(actHours, actMinutes);
                                ...
                                return (
                                    ...
                                    >
                                        {/* Accordion Header */}
                                        <div
                                            onClick={() => toggleExpandTicket(developer.developer_id)}
                                            className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 cursor-pointer hover:bg-[#FAFBFC] transition-colors select-none"
                                        >
                                            {/* Developer Info */}
                                            <div className="flex items-center gap-3.5 min-w-0 flex-1">
                                                <div className="w-10 h-10 rounded-xl bg-[#EAE6FF] text-[#403294] flex items-center justify-center font-bold text-sm shrink-0">
                                                    {developer.developer_name ? developer.developer_name.split(' ').map(n => n[0]).join('') : 'D'}
                                                </div>
                                                <div className="min-w-0">
                                                    <h4 className="font-bold text-[#172B4D] text-base truncate">{developer.developer_name}</h4>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex items-center justify-between md:justify-end gap-5 shrink-0">
                                                <div className="flex gap-4 text-right">
                                                    <div>
                                                        <div className="text-xs text-[#5E6C84] font-medium">Total Worked Time</div>
                                                        <div className="text-base font-bold text-[#172B4D]">{devTimeDisplay}</div>
                                                    </div>
                                                    <div>
                                                        <div className="text-xs text-[#5E6C84] font-medium">Total Actual Time</div>
                                                        <div className="text-base font-bold text-slate-600">{devActualTimeDisplay}</div>
                                                    </div>
                                                </div>
                                                <span className="text-xs font-bold text-[#6B778C] bg-[#F4F5F7] px-2.5 py-1 rounded-full shrink-0">
                                            ...
                                            <div className="border-t border-[#DFE1E6] bg-[#FAFBFC] p-5">
                                                <div className="overflow-hidden border border-[#DFE1E6] rounded-xl bg-white shadow-xs">
                                                    <table className="min-w-full divide-y divide-[#DFE1E6]">
                                                        <thead className="bg-[#FAFBFC]">
                                                            <tr>
                                                                <th scope="col" className="px-5 py-3 text-left text-xs font-bold text-[#8993A4] uppercase tracking-wider">Ticket</th>
                                                                <th scope="col" className="px-5 py-3 text-right text-xs font-bold text-[#8993A4] uppercase tracking-wider">Worked Time</th>
                                                                <th scope="col" className="px-5 py-3 text-right text-xs font-bold text-[#8993A4] uppercase tracking-wider">Actual Time</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody className="bg-white divide-y divide-[#DFE1E6]">
                                                            {(developer.tickets || []).map((ticket) => {
                                                                const { hours: tHrs, minutes: tMins } = parseWorkingHours(ticket.working_hours);
                                                                const ticketTimeDisplay = formatLoggedTime(tHrs, tMins);
                                                                const { hours: taHrs, minutes: taMins } = parseWorkingHours(ticket.actual_working_hours);
                                                                const ticketActualTimeDisplay = formatLoggedTime(taHrs, taMins);
                                                                    ...
                                                                    <tr key={ticket.ticket_id} className="hover:bg-[#FAFBFC] transition-colors">
                                                                        <td className="px-5 py-3.5 whitespace-nowrap text-sm font-semibold text-[#172B4D]">
                                                                            <div className="flex items-center gap-2">
                                                                                ...
                                                                                <span className="text-slate-500 font-normal truncate max-w-[300px]">
                                                                                    {ticket.title}
                                                                                </span>
                                                                            </div>
                                                                        </td>
                                                                        ...
                                                                        <td className="px-5 py-3.5 whitespace-nowrap text-sm text-slate-600 font-semibold text-right">
                                                                            {ticketActualTimeDisplay}
                                                                        </td>
                                                                    </tr>
                                                                );
                                                            })}
