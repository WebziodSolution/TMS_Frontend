...
import { getDailyReport, exportDailyReportExcel } from '../../services/reportService';
...
const DailyReports = ({ setAlert }) => {
    ...
    const handleExportExcel = async () => {
        if (!reportDate || !dayjs(reportDate).isValid()) {
            setAlert({ open: true, message: "Please select a valid date first.", type: "error" });
            return;
        }

        setExportLoading(true);
        try {
            const blob = await exportDailyReportExcel(reportDate.format('YYYY-MM-DD'));

            // Trigger download
            const url = window.URL.createObjectURL(new Blob([blob]));
            const link = document.createElement('a');
            link.href = url;
            const filename = `daily_report_${reportDate.format('YYYY-MM-DD')}.xlsx`;
    ...
    return (
        <div className="w-full max-w-7xl mx-auto space-y-8 animate-fade-in-up pb-10">
            ...
            {reportData.length === 0 ? (
                ...
                /* Simple Table View for Developer */
                <div className="space-y-4">
                    <h3 className="text-lg font-bold text-[#172B4D] px-1">Your Work Details</h3>
                    <div className="overflow-hidden border border-[#DFE1E6] rounded-2xl bg-white shadow-xs">
                        <table className="min-w-full divide-y divide-[#DFE1E6]">
                            <thead className="bg-[#FAFBFC]">
                                <tr>
                                    <th scope="col" className="px-5 py-3 text-center text-xs font-bold text-[#8993A4] uppercase tracking-wider border-l border-[#DFE1E6]">Project Name</th>
                                    <th scope="col" className="px-5 py-3 text-center text-xs font-bold text-[#8993A4] uppercase tracking-wider border-l border-[#DFE1E6]">Ticket No</th>
                                    <th scope="col" className="px-5 py-3 text-center text-xs font-bold text-[#8993A4] uppercase tracking-wider border-l border-[#DFE1E6]">Title</th>
                                    <th scope="col" className="px-5 py-3 text-center text-xs font-bold text-[#8993A4] uppercase tracking-wider border-l border-[#DFE1E6]">Worked Time</th>
                                    <th scope="col" className="px-5 py-3 text-center text-xs font-bold text-[#8993A4] uppercase tracking-wider border-l border-[#DFE1E6]">Actual Time</th>
                                </tr>
                            </thead>
                            <tbody className="bg-white divide-y divide-[#DFE1E6]">
                                {reportData.flatMap(dev => dev.tickets || []).map((ticket) => {
                                    ...
                                    const { hours: actHrs, minutes: actMins } = parseWorkingHours(ticket.actual_working_hours);
                                    const ticketActualTimeDisplay = formatLoggedTime(actHrs, actMins);

                                    return (
                                        <tr key={ticket.ticket_id} className="hover:bg-[#FAFBFC] transition-colors">
                                            <td className="px-5 py-3.5 whitespace-nowrap text-sm text-[#42526E] font-semibold text-center border-l border-[#DFE1E6]">
                                                {ticket.project_name || '-'}
                                            </td>
                                            <td className="px-5 py-3.5 whitespace-nowrap text-sm font-semibold text-[#0052CC] text-center border-l border-[#DFE1E6]">
                                                <Link to={`/dashboard/manage-tickets/view/${ticket.ticket_id}`} className="hover:underline">
                                                    {ticket.ticket_no}
                                                </Link>
                                            </td>
                                            <td className="px-5 py-3.5 text-sm text-[#172B4D] font-medium max-w-xs md:max-w-md truncate text-center border-l border-[#DFE1E6]">
                                                {ticket.title}
                                            </td>
                                            <td className="px-5 py-3.5 whitespace-nowrap text-sm text-[#172B4D] font-semibold text-center border-l border-[#DFE1E6]">
                                                {ticketTimeDisplay}
                                            </td>
                                            <td className="px-5 py-3.5 whitespace-nowrap text-sm text-slate-600 font-semibold text-center border-l border-[#DFE1E6]">
                            ...
                            <tr className="hover:bg-[#FAFBFC] transition-colors">
                                <td colSpan={3} className="px-5 py-3.5 whitespace-nowrap text-sm text-[#42526E] font-semibold text-center border-l border-[#DFE1E6]">
                                    Total
                                </td>
                                <td className="px-5 py-3.5 whitespace-nowrap text-sm text-[#42526E] font-semibold text-center border-l border-[#DFE1E6]">
                                    {totalTimeDisplay}
                ...
                /* Developers Accordion List for Manager/Admin */
                <div className="space-y-4">
